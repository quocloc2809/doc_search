...
export default function CommonTable({
  ...
  const minTableWidth = useMemo(() => Math.max(0, Number(tableWidth) || 0), [tableWidth])
  ...
  useEffect(() => {
    ...
    const updateRows = () => {
      ...
      const headerHeight = Math.ceil(headerElement?.getBoundingClientRect().height || 44)
      const measuredRowHeight = Math.ceil(firstRowElement?.getBoundingClientRect().height || fallbackRowHeight)
      const availableBodyHeight = Math.max(0, Math.floor(node.clientHeight - headerHeight))
      ...
      const baseRows = Math.floor((availableBodyHeight + fitOffset) / safeRowHeight)
  ...
  }, [autoPageSize, autoRowFitOffset, maxAutoRows, minAutoRows, rowHeightEstimate, safeData.length, safeColumns.length])
  ...
  }, [containerWidth, longColumns, minAutoColumnWidth, minTableWidth, responsive, safeColumns.length])
  ...
  if (safeColumns.length === 0) {
    return <div className="common-error">CommonTable cần ít nhất 1 cột</div>
  }
  ...
  const tableScrollStyle = autoPageSize || !tableMaxHeight
    ? undefined
  ...
  return (
    <div className="common-table-wrapper" ref={tableWrapperRef}>
      ...
      <div className="common-table-scroll" ref={tableScrollRef} style={tableScrollStyle}>
        <table className="common-table" style={{ width: `${renderedTableWidth}px` }}>
          <colgroup>
          ...
          <thead>
            <tr>
              {safeColumns.map((column, index) => (
                <th key={column.key || `head-${index}`}>{column.title || `Cột ${index + 1}`}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {pagedRows.length === 0 ? (
              <tr>
                <td colSpan={safeColumns.length}>{emptyText}</td>
              </tr>
            ) : (
              pagedRows.map((row, rowIndex) => (
                <tr key={row?.[rowKey] ?? `row-${rowIndex}`}>
                  {safeColumns.map((column, columnIndex) => {
                    ...
                    const isCustomNode = isValidElement(rawValue)

                    if (isCustomNode) {
                      return (
                        <td key={`${column.key || columnIndex}-${row?.[rowKey] ?? rowIndex}`}>
                          <div className="common-table-cell">{rawValue}</div>
                        </td>
                      )
                    }

                    const textValue = normalizeText(rawValue)
                    const isTooltipEnabled = column.tooltip !== false && textValue.length > 0
                    const lineClamp = Number(column.lineClamp) > 1 ? Number(column.lineClamp) : 1
                    const textClassName = lineClamp > 1 ? 'common-table-text common-table-text-multiline' : 'common-table-text'
                    const textStyle = lineClamp > 1 ? { WebkitLineClamp: lineClamp } : undefined
                        ...
                        <div className="common-table-cell">
                          {isTooltipEnabled ? (
                            <Tooltip content={textValue}>
                              <span className={textClassName} style={textStyle}>{textValue || '-'}</span>
                            </Tooltip>
                          ) : (
                            <span className={textClassName} style={textStyle}>{textValue || '-'}</span>
                          )}
      ...
      {showOverflowHint && layout.overflowWidth > 0 ? (
        <div className="common-pagination-info">
          Bảng đang vượt kích thước {layout.overflowWidth}px. Bạn có thể kéo ngang để xem đầy đủ.
        </div>
      ) : null}
      ...
      {pagination ? (
        <Pagination
          ...
          pageSizeOptions={autoPageSize ? [effectivePageSize] : pageSizeOptions}
